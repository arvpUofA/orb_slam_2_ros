 ...
 */
 ...
 Node::Node(const std::string &node_name,
     ...
     : rclcpp::Node(node_name, node_options), image_transport_(nullptr),
       map_points_publisher_(nullptr), pose_publisher_(nullptr),
       camera_info_sub_(nullptr), service_server_(nullptr),
   ...
   declare_parameter("pointcloud_frame_id",
                     rclcpp::ParameterValue(std::string("map")));
   declare_parameter("camera_frame_id",
                     rclcpp::ParameterValue(std::string("camera_link")));
   declare_parameter("map_file", rclcpp::ParameterValue(std::string("map.bin")));
   declare_parameter("voc_file",
                     rclcpp::ParameterValue(std::string("file_not_set")));
   ...
   declare_parameter(
       "prerectified",
       rclcpp::ParameterValue(false)); // if image is already rectivied
 }
 ...
 void Node::init(const ORB_SLAM2::System::eSensor &sensor) {
   ...
   compressed_image_publisher_ =
       create_publisher<sensor_msgs::msg::CompressedImage>(
           node_name_ + "/debug_image/compressed", 1);
   rendered_image_publisher_ =
       image_transport_->advertise(node_name_ + "/debug_image", 1);
   ...
   // Publish status
   status_publisher_ =
       create_publisher<orb_slam2_ros::msg::Status>(node_name_ + "/status", 10);
   ...
   camera_info_sub_ = create_subscription<sensor_msgs::msg::CameraInfo>(
       "/camera/camera_info", camera_info_qos,
       std::bind(&Node::cameraInfoCallback, this, std::placeholders::_1));
 
   service_server_ = create_service<orb_slam2_ros::srv::SaveMap>(
       node_name_ + "/save_map",
       std::bind(&Node::SaveMapSrv, this, std::placeholders::_1,
 ...
 void Node::Update() {
   if (!initialized_) {
     RCLCPP_WARN(get_logger(),
                 "Camera info not received, node has not been initialized!");
     return;
   }
 ...
 void Node::PublishPositionAsTransform(cv::Mat position) {
   if (publish_tf_param_) {
     ...
     tf2::impl::Converter<false, true>::convert(transform,
 ...
 void Node::PublishRenderedImage(cv::Mat image) {
   ...
   sensor_msgs::msg::CompressedImage::SharedPtr compressed_image_msg =
       cv_bridge::CvImage(header, "bgr8", compressed_image)
 ...
 tf2::Transform Node::TransformFromMat(cv::Mat position_mat) {
   ...
   // coordinate system
   const tf2::Matrix3x3 tf_orb_to_ros(0, 0, 1, -1, 0, 0, 0, -1, 0);
 ...
 Node::MapPointsToPointCloud(std::vector<ORB_SLAM2::MapPoint *> map_points) {
   // if (map_points.empty()) {
 ...
 #else
       // x. Do the transformation by just reading at the position of z instead
       // of x
       data_array[0] = map_points.at(i)->GetWorldPos().at<float>(2);
       // y. Do the transformation by just reading at the position of x instead
       // of y
       data_array[1] = -1.0 * map_points.at(i)->GetWorldPos().at<float>(0);
       // z. Do the transformation by just reading at the position of y instead
       // of z
 ...
 void Node::LoadOrbParameters(
   ...
   if (sensor_ == ORB_SLAM2::System::STEREO) {
     RCLCPP_INFO(get_logger(),
                 "Stereo, images must be rectified, use info.p, ignore info.d");
   ...
   if (sensor_ == ORB_SLAM2::System::STEREO ||
     ...
     // ROS camera calibration puts stereo baseline in the right camera info, not
     // left, so the sign is flipped. If we had right camera info we could do
     // this: parameters.baseline = -right_camera_info->p[3]
 #else
